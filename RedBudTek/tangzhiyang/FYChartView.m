...
- (void)drawRectangleAndVerticalText:(CGRect)rect
{
    ...
    for (int i = 1; i <= 5; i++)
    {
        ...
        NSString *text = [NSString stringWithFormat:@"%.2f", (i + 0) * (floorf(maxVerticalValue + 1)  / 5)];
...
- (void)drawValueLine:(CGRect)rect
{
    ...
    for (int i = 0; i < numberOfHorizontalItemCount; i++)
    {
        ...
        if (value < 999) {
            if (value*1.5 >= maxVerticalValue)  maxVerticalValue = value*1.5;
        }
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(chartView:horizontalTitleAtIndex:)])
    {
        for (int i = 0; i < valueItems.count; i++)
        {
            ...
            if (title)
            {
                ...
                if (i == 0) {
                    [title drawAtPoint:CGPointMake(5, rect.size.height - size.height) withFont:font];
                }
                else
                {
